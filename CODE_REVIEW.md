...
| ~~F4~~ | `host_protocol.cpp:53-80` + `state_machine.cpp:203-228` + `eeprom_test.cpp:45-52` | DONE (with deviations): `ProvisionRequest` struct (markUnset + has* checks) in eeprom_layout.h; host_protocol exposes one `provisionRequest()` instead of 7 accessors; `eepromFromProvision(req)` in eeprom_layout does the sentinel→default mapping (subsumes the spec's `eepromDefaults(hw)` — the third defaults site died with F8, so a standalone defaults fn had no second consumer); `eepromHeaderLooksBlank()` named (both eeprom_manager sites); `PADMAP_ID_SLOTS` delivered earlier as `AdapterBase::PADMAP_ID_COUNT` (LY8). provisionEeprom's stale "Mezzanine70 v1" log now prints the request's hw=. Wire behavior unchanged. | M |
