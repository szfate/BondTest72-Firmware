...
| DUT Pin → Adapter Pin | `adapter_pin = 71 − dut_pin` (assumed — see Notes) | DUT PCB (connector orientation) |
...
| 6  | IO  | 68 | 3  | IO  |
| 7  | IO  | 69 | 2  | IO  |
| 8  | VDD | 70 | 1  | VCC — measured |
| 9  | VDD | 70 | 1  | VCC — **not tested**: shares adapter pin 1 with die pad 8; one result covers both bonds |
| 10 | IO  | 1  | 70 | IO  |
| 11 | IO  | 2  | 69 | IO  |
| 12 | IO  | 3  | 68 | IO  |
| 13 | IO  | 4  | 67 | IO  |
...
| 52 | IO  | 41 | 30 | IO  |
| 53 | IO  | 42 | 29 | IO  |
| 54 | GND | 43 | 28 | GND — only GND pad with a routed adapter pin |
| 55 | IO  | 44 | 27 | IO  |
| 56 | IO  | 45 | 26 | IO  |
| 57 | IO  | 46 | 25 | IO  |
| 58 | IO  | 47 | 24 | IO  |
...
| 62 | IO  | 51 | 20 | IO  |
| 63 | IO  | 52 | 19 | IO  |
| 64 | IO  | 53 | 18 | IO  — extrapolated, see Notes |
| 65 | IO  | 54 | 17 | IO  — extrapolated, see Notes |
| 66 | IO  | 55 | 16 | IO  — extrapolated, see Notes |
| 67 | IO  | 56 | 15 | IO  — extrapolated, see Notes |
| 68 | IO  | 57 | 14 | IO  — extrapolated, see Notes |
| 69 | IO  | 58 | 13 | IO  — extrapolated, see Notes |
| 70 | IO  | 59 | 12 | IO  — extrapolated, see Notes |
| 71 | IO  | 60 | 11 | IO  — extrapolated, see Notes |
| 72 | IO  | 61 | 10 | IO  |
...
```
gndPin = 28  // adapter pin 28 (tester ch 27), die pad 54 — the only GND connection
```
...
ioAdapterPinsInRingOrder = [
   9,  8,  7,  6,  5,  4,  3,  2,          // die pads  0– 7
  70, 69, 68, 67, 66, 65, 64,              // die pads 10–16
  63, 62, 61, 60, 59, 58, 57, 56, 55, 54,  // die pads 18–27
  53, 52, 51, 50, 49, 48, 47, 46,          // die pads 28–35
  45, 44, 43, 42, 41, 40, 39, 38, 37, 36,  // die pads 37–46
  35, 34, 33, 32, 31, 30, 29,              // die pads 47–53
  27, 26, 25, 24, 23, 22, 21, 20, 19,      // die pads 55–63
  18, 17, 16, 15, 14, 13, 12, 11,          // die pads 64–71 (extrapolated)
  10                                       // die pad  72
]
```
...
(0.3 V threshold) is impossible. The VDD↔GND substrate-diode path (apin 1)
was tried first and REJECTED: it conducts weakly and unpredictably
(sometimes ≥1.5 V, sometimes not at all — likely not a clean substrate tie).
The working scheme uses an IO pad's lower ESD diode instead: force the GND
pad positive and sink die pad 10 (apin 70, an IO pad) — the GND→IO diode
forward-conducts hard and, per bench data, more stably than any other path.

```
presencePadA       = 28   // GND pad — forced + Kelvin-sensed
presencePadB       = 70   // die pad 10 (IO) — sunk
...
- **Die pad 9 is not tested** — it shares adapter pin 1 with die pad 8
...
- **Die pads 64–71 are extrapolated**: the verified run dp55→apin 27 …
  dp63→apin 19 continues `apin = 82 − dp` through dp64→apin 18 … dp71→apin 11,
  and dp72→apin 10 confirms the pattern at the far end. Verify against the
...
- DUT Pin column assumes the standard `adapter_pin = 71 − dut_pin` connector
  orientation; it is reference only.
