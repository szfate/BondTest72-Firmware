...
// Pack EepromData into 36 bytes (32 header + 4 CRC-32). buf must be at least 36 bytes.
...
void eepromSerialize(const EepromData& data, uint8_t buf[36]);

// Unpack 36 bytes into EepromData. Returns false if magic sentinel or CRC-32 is wrong.
bool eepromDeserialize(const uint8_t buf[36], EepromData& out);
